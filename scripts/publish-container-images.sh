...
if [ "$MODE" == "butler-only" ] || [ "$MODE" == "full-install" ]; then
  cd cf-butler
  pack build ${DOCKER_HOST}/cftoolsuite/cf-butler:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/cf-butler:${IMAGE_TAG} \
    --path . \
...
if [ "$MODE" == "hoover-only" ] || [ "$MODE" == "full-install" ]; then
  cd cf-hoover
  pack build ${DOCKER_HOST}/cftoolsuite/cf-hoover:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/cf-hoover:${IMAGE_TAG} \
    --path . \
...
if [ "$MODE" == "hoover-only" ] || [ "$MODE" == "full-install" ]; then
  cd cf-hoover-ui
  pack build ${DOCKER_HOST}/cftoolsuite/cf-hoover-ui:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/cf-hoover-ui:${IMAGE_TAG} \
    --path . \
...
if [ "$MODE" == "archivist-only" ] || [ "$MODE" == "full-install" ]; then
  cd cf-archivist
  pack build ${DOCKER_HOST}/cftoolsuite/cf-archivist:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/cf-archivist:${IMAGE_TAG} \
    --path . \
    --env BP_MAVEN_BUILD_ARGUMENTS="clean verify --batch-mode -DskipTests" \
...
if [ "$MODE" == "support-only" ] || [ "$MODE" == "full-install" ]; then
  ...
  pack build ${DOCKER_HOST}/cftoolsuite/config-server:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/config-server:${IMAGE_TAG} \
    --path . \
    --env BP_JVM_VERSION=21.* \
    --builder paketobuildpacks/builder-jammy-tiny \
    --volume $HOME/.m2:/home/cnb/.m2:rw \
...
if [ "$MODE" == "support-only" ] || [ "$MODE" == "full-install" ]; then
  ...
  pack build ${DOCKER_HOST}/cftoolsuite/discovery-service:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/discovery-service:${IMAGE_TAG} \
    --path . \
    --env BP_JVM_VERSION=21.* \
    --builder paketobuildpacks/builder-jammy-tiny \
    --volume $HOME/.m2:/home/cnb/.m2:rw \
...
if [ "$MODE" == "support-only" ] || [ "$MODE" == "full-install" ]; then
  ...
  pack build ${DOCKER_HOST}/cftoolsuite/microservices-console:latest \
    --tag ${DOCKER_HOST}/cftoolsuite/microservices-console:${IMAGE_TAG} \
    --path . \
    --env BP_JVM_VERSION=21.* \
    --builder paketobuildpacks/builder-jammy-tiny \
    --volume $HOME/.m2:/home/cnb/.m2:rw \
